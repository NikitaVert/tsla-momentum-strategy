...
├── notebooks/
│   ├── 01_data_and_signals.ipynb
│   ├── 02_backtest.ipynb
│   └── 03_performance_and_validation.ipynb
└── writeup/
...
2. Run the notebooks in order: `01` → `02` → `03`. Each notebook saves its
   output to a pickle file that the next notebook loads.
3. Google Colab works fine — just upload the `src/` folder alongside the
   notebooks (or `pip install` from the repo) so the imports resolve.
...
- **Entry trigger:** 20-day EMA crosses above the 50-day EMA, while in the
  bullish regime, RSI(14) is between 50–75, and volume is above its 20-day average.
...
This is a long/flat-only model — it does not take short positions in
